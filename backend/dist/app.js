...
// ──────────────────────────────────────────────
const saasLockMiddleware = async (req, res, next) => {
    if (!['POST', 'PUT', 'DELETE'].includes(req.method))
        return next();
    const url = req.originalUrl;
    const exempted = url.startsWith('/api/auth') ||
    ...
    try {
        ...
        logger.error('SaaS lock check failed', { error: err.message });
    }
    next();
};
app.use(saasLockMiddleware);
// ──────────────────────────────────────────────
// Health Check
// ──────────────────────────────────────────────
