...
export const tokenizeAndStem = (text) => {
    return text
        .toLowerCase()
    ...
    while ((matches = amountRegex.exec(clean)) !== null) {
        ...
        if (multiplier) {
            if (multiplier.startsWith('k'))
                val *= 1000;
            if (multiplier.startsWith('lakh') || multiplier.startsWith('lac'))
                val *= 100000;
        }
        extractedAmount = val;
    }
...
export class ChatbotEngine {
    ...
    trainEngine(intentsHash) {
        ...
        this.w1 = Array.from({ length: hiddenSize }, () => Array.from({ length: inputSize }, () => (Math.random() - 0.5) * Math.sqrt(2.0 / inputSize)));
        this.b1 = new Array(hiddenSize).fill(0.01);
        this.w2 = Array.from({ length: outputSize }, () => Array.from({ length: hiddenSize }, () => (Math.random() - 0.5) * Math.sqrt(2.0 / hiddenSize)));
    ...
    predict(vector) {
        if (!this.isTrained)
            return { tag: 'help', confidence: 1.0 };
    ...
    calculateAttentionSimilarities(stemmedWords) {
        const scores = {};
        for (const intent of this.intents) {
            let maxSim = 0;
            for (const pattern of intent.patterns) {
    ...
    handleMessage(message, context, role) {
        const stemmed = tokenizeAndStem(message);
        const entities = extractEntities(message);
        // Save context
        if (entities.amount !== null)
            context.lastMentionedAmount = entities.amount;
        if (entities.plan !== null)
            context.lastMentionedPlan = entities.plan;
        if (entities.section !== null)
            context.lastMentionedSection = entities.section;
        // Vectorize
