...
exports.deleteTrainer = exports.updateTrainer = exports.getTrainerById = exports.getTrainers = exports.createTrainer = void 0;
...
const createTrainer = async (req, res) => {
    try {
        ...
        if (email) {
            const existing = await prisma_js_1.default.trainer.findUnique({ where: { email } });
            if (existing) {
                return res.status(400).json({ error: 'Email already registered for another trainer' });
            }
        }
        const trainer = await prisma_js_1.default.trainer.create({
            data: {
                firstName,
                lastName,
                specialty,
...
const getTrainers = async (req, res) => {
    try {
        const trainers = await prisma_js_1.default.trainer.findMany({
...
const getTrainerById = async (req, res) => {
    try {
        const { id } = req.params;
        const trainer = await prisma_js_1.default.trainer.findUnique({
            where: { id },
        });
        if (!trainer) {
...
const updateTrainer = async (req, res) => {
    try {
        ...
        const trainer = await prisma_js_1.default.trainer.update({
            where: { id },
            data,
        });
...
const deleteTrainer = async (req, res) => {
    try {
        ...
        const classCount = await prisma_js_1.default.groupClass.count({
            where: { trainerId: id },
        });
        if (classCount > 0) {
        ...
        await prisma_js_1.default.trainer.delete({
            where: { id },
        });
