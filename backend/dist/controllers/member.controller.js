...
import { cacheGet, cacheSet, cacheDelPrefix, CacheKeys } from '../config/cache.js';
...
const invalidateMemberCaches = async (reqUser) => {
    if (!reqUser)
        return;
...
const checkMemberAccess = async (reqUser, memberId) => {
    if (!reqUser) {
...
export const addSubscriptionToMember = async (req, res) => {
    try {
        ...
        // 5. Notify the member
        await createNotification(member.userId, 'Membership Assigned', `A new membership plan "${plan.name}" has been added to your account. It is valid until ${new Date(result.subscription.endDate).toLocaleDateString()}.`, 'BILLING');
        await invalidateMemberCaches(reqUser);
        return res.status(201).json({
...
export const logMemberCheckIn = async (req, res) => {
    try {
        const { id: memberId } = req.params;
        const member = await prisma.member.findUnique({
            where: { id: memberId },
        ...
        const activeSub = member.subscriptions.find(sub => new Date(sub.startDate) <= new Date() &&
            new Date(sub.endDate) > new Date()) || member.subscriptions[0];
        ...
        await createNotification(member.userId, 'Check-in Logged', `Welcome back, ${member.user.firstName}! Your check-in was logged successfully.`, 'INFO');
        await invalidateMemberCaches(reqUser);
        return res.status(201).json({
...
export const getDashboardStats = async (req, res) => {
    try {
        ...
        const now = new Date();
        const [totalMembers, revenueResult, checkInsToday, pendingResult, recentCheckIns, membersInLastWeek, checkInsLastMonth, activeTrainers, expiringIn7Days] = await Promise.all([
            prisma.member.count({
                where: {
...
export const freezeMember = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await createNotification(member.userId, 'Membership Frozen', `Your membership has been frozen from ${new Date(startDate).toLocaleDateString()} to ${new Date(endDate).toLocaleDateString()}.`, 'ALERT');
        await invalidateMemberCaches(reqUser);
...
export const unfreezeMember = async (req, res) => {
    try {
        const { id } = req.params;
        const reqUser = req.user;
        const access = await checkMemberAccess(reqUser, id);
        ...
        await createNotification(member.userId, 'Membership Unfrozen', 'Your membership has been successfully unfrozen. Welcome back to active workouts!', 'ALERT');
        await invalidateMemberCaches(reqUser);
...
export const upgradeDowngradeSubscription = async (req, res) => {
    try {
        const { id: memberId } = req.params;
        const { planId } = req.body;
        const reqUser = req.user;
