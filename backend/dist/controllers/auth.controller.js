...
export const register = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: result.user.id, email: result.user.email, role: result.user.role, branchId: result.user.branchId }, JWT_SECRET, { expiresIn: '24h' });
        const { passwordHash: _, ...userWithoutPassword } = result.user;
        // Send welcome email (non-blocking)
        if (userRole === UserRole.ADMIN) {
            sendWelcomeEmail(result.user.email, result.user.firstName, 30).catch((e) => logger.error('Failed to send welcome email', { error: e.message }));
        }
...
export const login = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: user.id, email: user.email, role: user.role, branchId: user.branchId }, JWT_SECRET, { expiresIn: '24h' });
    ...
    try {
        ...
        if (!user) {
            ...
            if (email && !email.endsWith('@gymos.com')) {
                sendWelcomeEmail(email, firstName).catch((err) => logger.error('Error sending welcome email on social register', { error: err.message }));
            }
