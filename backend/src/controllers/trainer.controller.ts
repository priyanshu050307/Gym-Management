...
export const getTrainers = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const trainersWithRatings = trainers.map(t => {
      const feedbacks = (t as any).feedbacks || [];
      const ratingSum = feedbacks.reduce((sum: number, f: any) => sum + f.rating, 0);
      const averageRating = feedbacks.length > 0 ? (ratingSum / feedbacks.length) : 0;
      return {
        ...t,
        averageRating: Number(averageRating.toFixed(1)),
        feedbackCount: feedbacks.length,
        ptMembersCount: (t as any)._count?.ptMembers ?? 0,
      };
    });
...
export const updateTrainer = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (trainer.userId) {
      ...
      if (email !== undefined) userData.email = email || undefined;
      if (branchId !== undefined && req.user?.role === 'ADMIN') userData.branchId = branchId || null;
...
export const deleteTrainer = async (req: Request, res: Response) => {
  try {
    ...
    for (const ptMember of (trainer as any).ptMembers || []) {
      const { createNotification } = await import('./notification.controller.js');
      await createNotification(
        ptMember.userId,
        'Trainer Removed',
        `Your personal trainer ${trainerFullName} has been removed from the system. Please contact the front desk to arrange a new trainer assignment.`,
        'ALERT'
      );
    }
    ...
    return res.status(200).json({
      ...
      affectedMembers: (trainer as any).ptMembers?.length ?? 0,
    });
  } catch (error: any) {
...
export const getMyPTMembers = async (req: Request, res: Response) => {
  try {
