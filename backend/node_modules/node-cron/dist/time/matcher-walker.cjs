...
class MatcherWalker {
    ...
    matchNext() {
        ...
        for (let i = 0; i < MAX_DAYS; i++) {
            if (months.includes(month) && dayOfMonth.matchesDayOfMonth(days, year, month, day)) {
            ...
            for (const minute of minutes) {
                for (const second of seconds) {
                    if (lowerBound && !isLaterInDay(hour, minute, second, lowerBound))
                        continue;
                    const ts = localizedTime.localTimeToTimestamp({ year, month, day, hour, minute, second, milisecond: 0 }, this.timezone);
